...
async def etat_page(request: Request):
    ...
    """
    capteurs_actionneurs = db_request(query)
    return templates.TemplateResponse("etat.html", {
        "request": request,
        "capteurs_actionneurs": capteurs_actionneurs
    })

@app.get("/economie", response_class=HTMLResponse)
async def economie_page(request : Request):
    return templates.TemplateResponse("economie.html", {"request": request})
